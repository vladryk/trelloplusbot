...
def process_start_param(message: Message or None):
    if not message or not message.text:
        return
    if isinstance(message.text, str) and message.text.find('/start') == 0:
        try:
        ...
        if param and param.startswith('-'):
            ...
            message.text = '/' + param[1:]
            raise RestartHandler()
...
class NextHandler(Exception):
    """
    Небходимо бросить это исключение обработщиком команды в случае, если необходимо продолжить поиск подходящего обработщика.
    """
    pass


class RestartHandler(Exception):
    """
    Небходимо бросить это исключение обработщиком команды в случае, если необходимо начать с начала поиск подходящего обработщика.
    """
    pass
...
class ParamsErrorHandler(BaseErrorHandler):
    pass
...
def replace_html_entities(text: str) -> str:
    return multiple_replace(HTML_ENTITIES, text)


def multiple_replace(dic: dict, text: str) -> str:
    pattern = '|'.join(map(re.escape, dic.keys()))
    return re.sub(pattern, lambda m: dic[m.group()], text)


def keyboard_factory(tguser, keyboard, reply_markup=True):
    from bot.models import TgUser
    from bot.keyboards import Keyboard
    if not isinstance(tguser, TgUser):
        raise Exception('cannot send keyboard not to TgUser')
    if isinstance(keyboard, type) and issubclass(keyboard, Keyboard):
        keyboard = keyboard(tguser)
    if not isinstance(keyboard, Keyboard):
        raise Exception('keyboard (%s) is not instance of keyboards.Keyboard (%s)' % (type(keyboard), keyboard.__class__))
    if reply_markup:
        return keyboard.get_reply_markup()
    return keyboard
...
def render_to_string(template_name, context=None):
    text = loader.render_to_string(template_name, context=context)
    return prepare_text(text)


def render_from_string(template_code, context=None):
    template = engines['django'].from_string(template_code)
    assert isinstance(template, Template)
    return prepare_text(template.render(context))


def bot_url(start: str = ''):
    if start:
        start = base_utils.real_urlsafe_b64encode(start.encode())
        start = '?start=%s' % urlencode(start)
    return 'https://telegram.me/%s' % settings.TELEGRAM_BOT_NAME + start


class TrelloClient(trolly.Client):
    def __init__(self, tguser, user_auth_token):
        super().__init__(settings.TRELLO_API_KEY, user_auth_token)
        self.tguser = tguser
    ...
    def get_authorisation_url(self):
        query_params = {
            ...
            'return_url': base_utils.site_url(reverse('token')),
        }
    ...
    def check_errors(self, uri, response):
        try:
            return super().check_errors(uri, response)
        except trolly.ResourceUnavailable:
            self.tguser.unauthorized()
            raise StateErrorHandler('unauthorized')
